    ...
    for record in event["Records"]:
        ...
        for row in reader:
            try:
                ...
                #
                # NOTE: app.py already rejects invalid data before it ever
                # uploads to S3, using this same Student class - so today,
                # a bad row reaching this Lambda shouldn't happen. Keeping
                # this check anyway because that guarantee only holds while
                # app.py is the ONLY producer writing to this bucket and
                # stays in sync with student.py. If a bulk import, manual
                # re-upload (e.g. from a future quarantine fix), or a
                # version drift between app.py and this handler ever
                # happens, this becomes a live path again. Don't remove
                # this thinking it's dead code - it's a safety net, not
                # the primary filter.
